---
id: concept-obsidian
type: concept
title: Obsidian
tags: [obsidian, notes, markdown]
source_count: 2
summary: Obsidian is the human-facing interface for navigating the repo’s markdown corpus, wiki links, and mixed raw-versus-derived knowledge artifacts.
canonical_for: [obsidian workflow]
review_status: reviewed
last_reviewed: 2026-05-18
review_due: 2026-06-18
confidence: "0.72"
---

# Obsidian

## Summary

Obsidian is used here as the human-facing frontend for browsing raw sources, compiled concept pages, and generated artifacts like slides or plots.

## Why It Fits

- local markdown-first workflow
- wiki-link navigation
- good support for mixed notes and derived artifacts
- useful as the browsing layer even when indexing and synthesis happen outside Obsidian itself

## Source Notes

- [[2026-04-08-llm-knowledge-bases]]
- [[2026-04-09-momo-research-repo-overview]]
