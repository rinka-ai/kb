---
id: article-2026-04-16-resolvers-the-routing-table-for-intelligence
type: source
title: "Resolvers: The Routing Table for Intelligence"
path: raw/articles/2026-04-16-resolvers-the-routing-table-for-intelligence.md
author:
publisher: User-provided text
url:
date_published:
date_added: 2026-04-16
tags: [agents, routing, context-engineering, memory, skills, governance, evals]
status: processed
quality: medium
...
The essay argues that practical agent systems need explicit resolver documents that map user tasks and content types to the right skills, directories, and context sources. A small routing layer beats a huge omnibus instructions file because it keeps the model's active context clean, makes capabilities reachable, and creates a governance surface that can be tested, audited, and eventually improved from traffic.

## Key Claims

- A resolver is a routing table for context: when task type X appears, load document or skill Y first.
- Oversized instruction files degrade model performance by polluting the context window with information that should load only on demand.
- Every skill that writes to memory or a knowledge repo should consult shared resolver and filing rules instead of inventing local defaults.
- Silent misfiling is more dangerous than obvious hallucination because it gradually turns structured memory into an unusable junk drawer.
- Skill routing should be evaluated directly with trigger evals rather than only validating end outputs.
- Reachability matters: a skill that exists but has no path from the resolver is effectively a dead capability.
- Resolver tables decay over time as new skills appear and user phrasing changes, so they need recurring audits and maintenance.
- The long-term goal is a self-healing resolver that updates routing rules from observed dispatch traffic.
- Resolvers compose at multiple layers: top-level skill routing, filing decisions inside the memory repo, and sub-routing inside skills.
- The deepest framing is organizational: skills act like employees, the resolver acts like the org chart, filing rules act like process, and reachability checks act like audit or compliance.

## Important Details

- The author contrasts a 20,000-line `CLAUDE.md` with a roughly 200-line decision tree that points to the right directories and documents on demand.
- A misfiled policy essay about OpenAI industrial policy is used to show how hardcoded defaults inside ingest skills cause systematic memory corruption.
- An audit found that only 3 of 13 brain-writing skills consulted the resolver, while the rest used hardcoded destination paths.
- The proposed fix is a shared filing rules document plus a universal requirement that every brain-writing skill read `RESOLVER.md` before creating a page.
- The essay highlights an "invisible skill" failure mode where a well-built capability exists but cannot be triggered because the resolver lacks the right descriptions.
- The suggested test method is a resolver trigger eval suite with example user inputs and expected skill dispatch targets.
- The proposed `check-resolvable` meta-skill inspects the chain from `AGENTS.md` to skill file to codepath and flags capabilities that are unreachable from current routing.
- The author reports six unreachable skills out of more than forty, including flight tracking, content ideas, and citation fixing.
- Resolver decay is described as a form of context rot caused by skill growth, changing user phrasing, and bypassing the routing layer with direct instructions.
- The forward-looking idea is an RLM-like loop that rewrites resolver triggers based on observed dispatches, missed matches, and overlaps.
- The piece situates GBrain as the memory layer, GStack as the coding and skills layer, and OpenClaw or Hermes Agent as the thin harness that orchestrates them.

## Entities

- Concepts: resolvers, routing tables, context rot, trigger evals, reachability, self-healing routing, thin harness, fat skills, filing rules, governance layer
- Artifacts: `CLAUDE.md`, `AGENTS.md`, `RESOLVER.md`, `_brain-filing-rules.md`, `check-resolvable`
- Systems: GBrain, GStack, OpenClaw, Hermes Agent, cron jobs, knowledge repos
- People and orgs mentioned: Will Manidis, OpenAI, YC

## My Notes

- This is best treated as a builder doctrine essay rather than as a neutral empirical source. Its value comes from the operating pattern it names: explicit routing as a first-class surface.
- The strongest reusable contribution is not the exact file names, but the split between always-on root instructions and on-demand resolver-guided context loading.
- The essay fits naturally with existing KB material on agent skills, thin harnesses, context rot, and externalized memory, but it adds a sharper governance and reachability lens.
- The self-healing resolver idea is forward-looking and more speculative than the filing rules or trigger-eval recommendations.

## Open Questions

- Should the KB promote `resolvers` to a standalone concept page, or should the idea stay embedded under context engineering, skills, and harnesses?
- Which existing notes best triangulate the resolver framing beyond this essay's anecdotal evidence?
- How should we distinguish resolver logic from adjacent concepts like protocol routing, retrieval, workflow orchestration, and memory schemas?
- If we later verify provenance, should this note be superseded by a version with canonical source URL, publication date, and confirmed authorship?
