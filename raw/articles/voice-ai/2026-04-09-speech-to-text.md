---
id: article-2026-04-09-speech-to-text
type: source
title: "Speech to text"
path: raw/articles/voice-ai/2026-04-09-speech-to-text.md
author: Unknown
publisher: platform.openai.com
url: https://platform.openai.com/docs/guides/speech-to-text
date_published: 
date_added: 2026-04-09
tags: [voice-ai, speech-to-text, transcription, audio, input]
status: processed
quality: high
summary: "OpenAI's speech-to-text guide covers transcription and translation endpoints, newer transcription models including diarization support, supported file formats, and output-format tradeoffs."
related: [voice-ai, speech-to-text, transcription, audio, input]
---
...
OpenAI's speech-to-text guide is the core input-side reference for voice systems: it covers the transcription and translation endpoints, model options from Whisper through newer `gpt-4o` transcribe variants, diarization support, file-size limits, and output-format differences.

## Key Claims

- OpenAI exposes both transcription and translation flows for speech input.
- The guide distinguishes older `whisper-1` support from newer `gpt-4o` transcription models.
- Diarization is available as a separate capability when speaker separation matters.
- Practical implementation constraints such as file-size limits, supported media types, and output formats shape product design as much as model choice does.

## Important Details

- The guide lists `gpt-4o-mini-transcribe`, `gpt-4o-transcribe`, and `gpt-4o-transcribe-diarize` as supported higher-quality options on the transcriptions endpoint.
- File uploads are limited to 25 MB.
- Supported input types include `mp3`, `mp4`, `mpeg`, `mpga`, `m4a`, `wav`, and `webm`.
- Output-format support varies by model, with diarized JSON available specifically for the diarization model.
- The translation path converts speech into English rather than merely transcribing it in the original language.

## Entities

- Company: OpenAI
- Products and APIs: Audio API, transcriptions endpoint, translations endpoint, whisper-1, gpt-4o-transcribe, gpt-4o-transcribe-diarize
- Concepts: speech-to-text, translation, diarization, output formats

## My Notes

- This is the implementation anchor for the input side of the voice stack.
- The diarization and output-format details are especially important for support, meeting, and multi-speaker use cases.

## Open Questions

- Where do we need speaker attribution strongly enough to justify diarization-first designs?
- Which downstream tasks need raw timestamps or structured JSON rather than plain text transcripts?
...
Using the prompt parameterThe first method involves using the optional prompt parameter to pass a dictionary of the correct spellings.Because it wasn’t trained with instruction-following techniques, Whisper operates more like a base GPT model. Keep in mind that Whisper only considers the first 224 tokens of the prompt.1
2
3
4
5
...
print(transcription.text)While it increases reliability, this technique is limited to 224 tokens, so your list of SKUs needs to be relatively small for this to be a scalable solution.
Post-processing with GPT-4The second method involves a post-processing step using GPT-4 or GPT-3.5-Turbo.We start by providing instructions for GPT-4 through the system_prompt variable. Similar to what we did with the prompt parameter earlier, we can define our company and product names.1
2
3
4
5
