...
source_count: 21
summary: Managed agents decouple model reasoning from durable runtime interfaces for sessions, runs, approvals, tools, credentials, inter-agent tasks, observability, entrypoints, and state.
canonical_for: [managed agents, hosted agent runtimes, resumable approvals, decoupling the brain from the hands, omnichannel agent architecture, messaging gateway agent runtime, unified inbox agents, human approval handoff architecture]
...
Managed agents are agent systems built around durable interfaces for state, execution, interruption, credentials, and orchestration so the implementation can change as models and harness techniques improve. Anthropic's Managed Agents post makes the core shape explicit: session logs, harnesses, and sandboxes should be separable interfaces, with model-context management treated as a replaceable harness concern rather than the durable source of truth. The newer runtime and protocol additions make that architecture more concrete: durable workflows, resumable approvals, explicit run/thread boundaries, stable external APIs, brokered credentials, and reusable entrypoint adapters matter as much as the model loop itself. A2A adds a cross-runtime discovery and task layer, while OpenTelemetry GenAI adds portable trace structure across agent, model, retrieval, tool, and MCP operations; together they make interoperability and operational evidence explicit without removing the need for local authorization and privacy policy. Hermes adds a personal-agent version: one runtime can sit behind terminal, messaging, editor, cron, API, library, and batch surfaces while sharing provider routing, memory, skills, sessions, tools, and delivery paths. The v0.19.1 teardown adds that provider/model identity, budgets, snapshot identity, cancellation, usage, and retry lineage belong to the durable run record rather than being implicit loop state. MemWal adds the user-owned memory surface: managed agents should be able to use durable memory without trapping all state inside the runtime vendor's database. Mukta's production-memory talk adds a managed learning loop: shared file-backed memory needs attributed version history, optimistic concurrency, scoped read/write permissions, portability, and a periodic job that converts selected session evidence into a reviewable candidate memory state.
...
- Shared organizational memory should be a managed capability with versioning, attribution, content-hash concurrency, rollback, and scope-aware read/write policy.
- Background consolidation should select session evidence through the same authority boundary as the target memory store and produce an inspectable candidate version before promotion.

## Design Shape

- `thread` or `session`: durable append-only state and history
- `run`: one execution with status, streaming, and resume semantics
- `run identity`: pinned model/provider, input snapshot, budgets, retry lineage, usage, and terminal outcome
...
- `dreaming or consolidation job`: a periodic run over permission-matched transcripts and tool metadata that clones, verifies, organizes, enriches, and proposes a successor memory store
...
- lets fleet-wide lessons compound across sessions without forcing each live agent to spend all of its task context maintaining shared memory
...
- fast in-band learning vs the cost, delay, and governance burden of cross-session consolidation
