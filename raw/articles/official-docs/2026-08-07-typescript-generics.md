---
id: 2026-08-07-typescript-generics
type: source
title: TypeScript Generics
path: raw/articles/official-docs/2026-08-07-typescript-generics.md
...
status: active
quality: high
summary: The official TypeScript handbook explains how generics preserve relations between types through inference, constraints, parameter placement and defaults, and why variance annotations are rare performance/debugging tools rather than a way to override structural behavior.
related: [typescript-generic-api-design, typescript-structural-compatibility, typescript-conditional-types, typescript-mapped-types, typescript-module-systems]
---
...
- Publisher: Microsoft
- URL: https://www.typescriptlang.org/docs/handbook/2/generics.html
- Acquisition: Captured through `bun run kb:ingest --url` on 2026-08-07; the complete endpoint-captured text is preserved below.

## TL;DR

Generics preserve relationships that `any` discards: the same type parameter can connect inputs, outputs, members, keys, constructors, and defaults while constraints expose only the capabilities an implementation may safely use. Public API quality depends on placing parameters at the right scope, keeping inference useful, publishing truthful declarations, and treating variance as structurally emergent rather than an annotation-controlled escape hatch.

## Key Claims

- A generic type parameter carries type information through a reusable component; unlike `any`, it can preserve a precise input-output relation.
- Type argument inference commonly removes the need for explicit type arguments, but callers may need to supply them when inference cannot establish the intended relation.
- Unconstrained parameters must be handled as potentially any type; `extends` constraints expose a required structural capability without discarding the caller's more specific type.
- A parameter on a call signature makes each call generic, while a parameter on an interface or class fixes one instantiation for all relevant members; generic classes parameterize only their instance side.
- Constraints can relate parameters, as in `Key extends keyof Type`, so a key and indexed result remain correlated.
- Generic defaults make trailing type arguments optional, must satisfy their constraints, and provide a fallback when inference finds no candidate.
- Covariance and contravariance emerge from structural positions. TypeScript infers variance and only consults explicit `in`/`out` annotations for some same-generic instantiation comparisons.
- Variance annotations must match structural behavior, cannot force invariance during ordinary structural comparison, and are justified only by specific debugging or measured checker-performance evidence.

## Important Details

- Constructor constraints such as `new () => A` connect a factory input's static constructor surface to its produced instance type.
- A generic parameter creates meaningful separation only when it participates in the resulting structure; phantom parameters do not produce nominal identity in TypeScript's structural system.
- Required type parameters cannot follow optional/defaulted parameters, and unspecified optional arguments resolve to defaults.
- Declaration merging can add a default to an existing parameter or add a new parameter only when the new parameter has a default.
- The handbook warns that whether a comparison uses instantiation-based variance optimization or full structural comparison is unspecified and may change for correctness or performance.
- The page supplies semantics and examples, not benchmarks, package declaration studies, runtime validation, or a compiler-version stability guarantee.
...
- Concepts: generic type parameters, type argument inference, constraints, `keyof`, constructor signatures, defaults, covariance, contravariance, invariance, structural comparison

## My Notes

- **Type/API boundary:** expose the smallest set of parameters that encodes a real relation. If callers must manually provide arguments that the runtime/API could infer, the abstraction may be over-parameterized.
- **Runtime/type boundary:** type arguments, constraints, defaults, and variance annotations are erased. Accept external values as `unknown`, validate them, and then apply generic relations to admitted domain values.
- **Package graph:** exported generic declarations are evaluated in downstream compilers. Test built/packed public imports across the supported TypeScript matrix rather than relying on monorepo source aliases.
- **Composition/lifecycle:** generic containers can describe value relations but do not own resource acquisition, disposal, retries, authorization, cancellation, or runtime registry membership.
- **Tests:** combine positive inference checks, deliberate negative cases, explicit-argument cases, declaration-consumer fixtures, and runtime tests for any implementation branch hidden behind an assertion.
- **Performance:** do not add variance annotations from intuition. Profile representative consumer checking/editor behavior, identify variance inference as the bottleneck, ensure the annotation matches structure, and retain regression evidence.
- **Practical implication:** prefer capability constraints and inference-preserving return relations over `any`, broad assertions, phantom identity parameters, or public generic machinery that merely mirrors implementation detail.

## Open Questions

- Which exported APIs in a given package need explicit compiler-version fixtures because inference quality is part of their compatibility promise?
- Where would a discriminated union, overload, opaque identity, runtime schema, or separate operation communicate intent better than another generic parameter?
- Which measured checker traces, if any, justify an explicit variance annotation in a real public declaration?
...
tsfunction loggingIdentity<Type>(arg: Type): Type {  console.log(arg.length);Property 'length' does not exist on type 'Type'.2339Property 'length' does not exist on type 'Type'.  return arg;}Try
...
tsfunction loggingIdentity<Type>(arg: Type[]): Type[] {  console.log(arg.length);  return arg;}Try
...
tsfunction loggingIdentity<Type>(arg: Array<Type>): Array<Type> {  console.log(arg.length); // Array has a .length, so no more error  return arg;}Try
...
tsinterface GenericIdentityFn {  <Type>(arg: Type): Type;}function identity<Type>(arg: Type): Type {  return arg;}let myIdentity: GenericIdentityFn = identity;Try
...
tsinterface GenericIdentityFn<Type> {  (arg: Type): Type;}function identity<Type>(arg: Type): Type {  return arg;}let myIdentity: GenericIdentityFn<number> = identity;Try
...
tsclass GenericNumber<NumType> {  zeroValue: NumType;  add: (x: NumType, y: NumType) => NumType;}let myGenericNumber = new GenericNumber<number>();myGenericNumber.zeroValue = 0;myGenericNumber.add = function (x, y) {  return x + y;};Try
...
tslet stringNumeric = new GenericNumber<string>();stringNumeric.zeroValue = "";stringNumeric.add = function (x, y) {  return x + y;};console.log(stringNumeric.add(stringNumeric.zeroValue, "test"));Try
...
tsinterface Lengthwise {  length: number;}function loggingIdentity<Type extends Lengthwise>(arg: Type): Type {  console.log(arg.length); // Now we know it has a .length property, so no more error  return arg;}Try
Because the generic function is now constrained, it will no longer work over any and all types:
tsloggingIdentity(3);Argument of type 'number' is not assignable to parameter of type 'Lengthwise'.2345Argument of type 'number' is not assignable to parameter of type 'Lengthwise'.Try
Instead, we need to pass in values whose type has all the required properties:
tsloggingIdentity({ length: 10, value: 3 });Try
...
tsfunction getProperty<Type, Key extends keyof Type>(obj: Type, key: Key) {  return obj[key];}let x = { a: 1, b: 2, c: 3, d: 4 };getProperty(x, "a");getProperty(x, "m");Argument of type '"m"' is not assignable to parameter of type '"a" | "b" | "c" | "d"'.2345Argument of type '"m"' is not assignable to parameter of type '"a" | "b" | "c" | "d"'.Try
...
tsdeclare function create(): Container<HTMLDivElement, HTMLDivElement[]>;declare function create<T extends HTMLElement>(element: T): Container<T, T[]>;declare function create<T extends HTMLElement, U extends HTMLElement>(  element: T,  children: U[]): Container<T, U[]>;Try
With generic parameter defaults we can reduce it to:
tsdeclare function create<T extends HTMLElement = HTMLDivElement, U extends HTMLElement[] = T[]>(  element?: T,  children?: U): Container<T, U>;const div = create();      const div: Container<HTMLDivElement, HTMLDivElement[]>const p = create(new HTMLParagraphElement());     const p: Container<HTMLParagraphElement, HTMLParagraphElement[]>Try
...
tsinterface Consumer<T> {  consume: (arg: T) => void;}
...
ts// Contravariant annotationinterface Consumer<in T> {  consume: (arg: T) => void;}// Covariant annotationinterface Producer<out T> {  make(): T;}// Invariant annotationinterface ProducerConsumer<in out T> {  consume: (arg: T) => void;  make(): T;}
...
ts// DON'T DO THIS - variance annotation// does not match structural behaviorinterface Producer<in out T> {  make(): T;}// Not a type error -- this is a structural// comparison, so variance annotations are// not in effectconst p: Producer<string | number> = {    make(): number {        return 42;    }}
...
ts// Error, this interface is definitely contravariant on Tinterface Foo<out T> {  consume: (arg: T) => void;}
