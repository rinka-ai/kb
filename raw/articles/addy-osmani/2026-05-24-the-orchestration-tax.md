---
id: article-2026-05-24-the-orchestration-tax
type: source
title: The Orchestration Tax
path: raw/articles/addy-osmani/2026-05-24-the-orchestration-tax.md
author: Addy Osmani
publisher: AddyOsmani.com
url: https://addyosmani.com/blog/orchestration-tax/
date_published: 2026-05-24
date_added: 2026-05-31
tags: [agents, multi-agent, orchestration, agentic-coding, code-review, attention, workflow-bottlenecks]
status: active
quality: high
summary: Addy Osmani argues that the practical limit on parallel AI coding agents is not agent startup cost but the human review, architectural judgment, and merge bandwidth that remain serial.
related: [multi-agent-systems, workflows, llm-agents, enterprise-agent-deployment-failure-modes]
---
...
Osmani frames multi-agent coding as a concurrent system with a single serial bottleneck: the developer's attention. Spawning agents is cheap, but steering them, reviewing diffs, reconciling conflicts, and preserving the system mental model all pass through one human reviewer, so throughput only improves when the workflow is designed around that scarce review capacity.

## Key Claims

- The hidden asymmetry in agentic coding is that launching agents is cheap while closing the loop on their output is expensive.
- The human operator is the serial resource in the system: architectural judgment, conflict resolution, and final merge decisions must "acquire the lock."
- Amdahl's Law applies to AI-agent workflows because total speedup is capped by the judgment and review work that cannot be parallelized.
- Adding more agents can optimize the non-bottleneck producer side while deepening the queue in front of human review.
- The tax is paid either deliberately through backpressure and review discipline, or silently through shallow reviews, cognitive surrender, stale mental models, and technical debt.
- Feeling busy with many active agents is not the same as shipping high-quality code to `main`.

## Important Details

- The post grew out of a Google I/O panel with Richard Seroter, Aja Hammerly, and Ciera Jaspan.
- Osmani uses Python's Global Interpreter Lock as an analogy: many agent "threads" can exist, but work requiring real understanding must pass through the one lock held by the human operator.
- The post explicitly connects the bottleneck to Amdahl's Law: if judgment is the serial fraction, increasing agent parallelism cannot linearly increase shipped work.
- Context switching across agents creates repeated cold reloads; the operator loses time rebuilding task state and never reloads it perfectly.
- Practical mitigations are to scale the fleet to review rate, delegate isolated work, avoid parallelizing judgment-heavy work, batch reviews, make agents prove routine correctness with tests or screenshots, and protect focused thinking time.
- The debt mechanism is subtle: poor reviews and stale understanding may not show up on the dashboard, but they surface when production breaks and the team no longer understands the system.

## Entities

- People: Addy Osmani, Richard Seroter, Aja Hammerly, Ciera Jaspan, Margaret-Anne Storey
- Organizations: Google, Google Cloud
- Concepts: orchestration tax, AI coding agents, human review bottleneck, Amdahl's Law, Global Interpreter Lock, cognitive surrender, technical debt, cognitive debt, backpressure, code review

## My Notes

- This is a compact but high-leverage source because it turns "multi-agent coordination overhead" into an explicit throughput model: agent production is parallel, but human judgment is serial.
- The strongest reusable rule is to scale background agents to review capacity rather than UI capacity.
- It pairs naturally with Cognition's "many minds, one writer" production pattern: extra agents should sharpen search, review, and proof, not create unreviewable parallel mutation streams.
- The source should influence coding-agent orchestration guidance more than generic agent-framework selection. It is about operator attention as an architectural resource.

## Open Questions

- What instrumentation best measures the review-rate bottleneck: open diffs per human, review latency, reverted-agent changes, conflict rate, stale-mental-model incidents, or production regressions?
- Which agent tools can make review cheaper without increasing cognitive surrender, for example by requiring tests, screenshots, trace summaries, or risk annotations?
- How should teams divide work between one human reviewer, multiple human reviewers, and automated verification when agent fleets become normal?
...
It’s what happens when you put a single-threaded resource in charge of a concurrent one.
...
The tiredness has a very specific cause. It is how running a serial processor at 100% with no slack feels like. Everytime you check on an agent you been away from you pay a context switch cost. You flush your brain and reload a different context from cold. CPUs do this in microseconds and architects still work hard to avoid it. You do it in minutes and you never reload the context perfectly. Five agents is not 1x workload done five times.

It is 5 cold reloads plus a background brain process constantly worrying about which agent you should be checking.
...
It just thrashes the lock and everything comes out worse.
