---
id: 2026-08-01-typescript-modules-reference
type: source
title: TypeScript Modules Reference
path: raw/articles/official-docs/2026-08-01-typescript-modules-reference.md
author: TypeScript Team
publisher: Microsoft
url: https://www.typescriptlang.org/docs/handbook/modules/reference.html
date_published:
date_added: 2026-08-01
tags: [typescript, modules, module-resolution, nodejs, bun, esm, commonjs, package-exports, monorepos, declaration-files]
status: active
quality: high
summary: The official TypeScript modules reference explains why compiler module settings must model the target runtime or bundler, how type lookup shadows runtime resolution, and how package exports, declarations, and workspaces determine whether typed imports remain valid after compilation.
related: [typescript-module-systems, codebase-architecture, internal-engineering-conventions]
---
...
- Path: raw/articles/official-docs/2026-08-01-typescript-modules-reference.md
- Author: TypeScript Team
- Published: Unknown; this is a continuously maintained handbook reference.
- Publisher: Microsoft
- URL: https://www.typescriptlang.org/docs/handbook/modules/reference.html
- Provenance: Captured through `bun run kb:ingest` on 2026-08-01, then relocated into the established `official-docs` collection and curated without changing `## Source Text`.

## TL;DR

TypeScript module checking is a model of a host—Node.js, Bun, a bundler, or another loader—not a standalone resolver. Correctness requires aligning `module` and `moduleResolution` with that host, preserving the distinction between erased types and runtime values, and testing packages through real workspace/package resolution rather than aliases that bypass `package.json` contracts.

## Key Claims

- TypeScript recognizes ECMAScript modules plus selected CommonJS and TypeScript-specific forms, but type-only imports and exports are guaranteed to disappear from emitted JavaScript.
- `node16`, `node18`, `node20`, and `nodenext` model Node's dual ESM/CommonJS system; they do not mean “ESM-only.” File extensions and the nearest `package.json` `type` field determine each file's format.
- `moduleResolution` should match the resolver used by the target runtime or bundler. Even under `noEmit`, hypothetical emit affects condition selection and therefore the types TypeScript exposes.
- TypeScript first imitates the host's runtime lookup, then substitutes `.ts`, `.tsx`, or declaration extensions so compile-time types correspond to the JavaScript path the host will load.
- `package.json` `exports` is an API boundary: it selects `import`/`require` and `types` conditions and blocks undeclared subpaths. Declaration files must accurately describe the runtime branch selected by the same request.
- `paths` does not rewrite emitted imports. It can make a project type-check while failing at runtime, and should not point at sibling monorepo packages or installed packages because doing so bypasses normal package resolution and `exports` behavior.
- Monorepos should use package-manager workspaces so local consumers exercise the same package graph and `node_modules` lookup that external consumers will use.

## Important Details

- **Type/API boundary:** `import type` and `export type` make erasure explicit. Ambient declarations describe runtime-provided modules but do not create implementations. `.d.mts`, `.d.cts`, and `.d.ts` participate in different module-format contracts.
- **Runtime-versus-type boundary:** a successful type lookup is only sound when the runtime resolves the same specifier to the implementation represented by that declaration. CommonJS named imports and `require(esm)` with transitive top-level await are documented places where TypeScript can be optimistic or incomplete.
- **Package graph:** conditional `exports`, subpath allowlists, package `type`, `imports`, self-name imports, `typesVersions`, workspace symlinks, and file extensions jointly define the public graph. Deep imports blocked by `exports` are intentionally outside the package API.
- **Composition/lifecycle ownership:** application composition must choose settings from the actual execution host. Node applications use a Node mode; bundlers and raw-TypeScript runtimes use the matching bundler/preserve behavior. A reusable library cannot force consumers to reproduce private aliases.
- **Tests:** test both type checking and execution/package consumption. For libraries, pack or link the built artifact into a consumer fixture and exercise ESM and CommonJS branches when both are published; for monorepos, resolve through workspaces rather than source aliases.
- **Performance:** the reference makes no benchmark claims. Resolution configuration can reduce ambiguity, but performance effects require compiler traces or measurements and should not be inferred from this document.
- **Transfer limits:** `bundler` intentionally permits resolution patterns that Node ESM rejects, while `nodenext` tracks current stable Node behavior and can change. A configuration valid for Bun, `tsx`, or a bundler is not automatically valid for direct Node execution or for a published package.
- **Practical implication:** treat `tsconfig`, file extensions, and `package.json` module fields as one executable compatibility contract. Review them together whenever runtime, packaging, or workspace structure changes.

## Entities

- Organization: Microsoft TypeScript Team
- Runtimes and hosts: Node.js, Bun, JavaScript bundlers, SystemJS, RequireJS
- Package metadata: `package.json` `type`, `exports`, `imports`, `main`, `types`, `typesVersions`
- Compiler options: `module`, `moduleResolution`, `paths`, `baseUrl`, `customConditions`, `verbatimModuleSyntax`, `noEmit`
- Module forms: ECMAScript modules, CommonJS, type-only imports/exports, ambient modules, declaration files

## My Notes

- Use runtime-first configuration: identify who executes or bundles the emitted/raw code, then select the TypeScript mode that models that host.
- Treat package export maps and declarations as executable API design, not release metadata added after implementation.
- Prefer workspace/package-boundary consumer tests over `paths` shortcuts. Aliases are acceptable only when the runtime or bundler independently implements the same mapping.
- This source establishes the first dedicated TypeScript language/module foundation in the KB; adjacent runtime-specific docs should refine, not override, its host-matching rule.

## Open Questions

- Which package-consumer fixture gives the smallest reliable ESM/CommonJS/declaration compatibility test for libraries in this KB's preferred Bun/Hono stack?
- When should a dual-format package accept duplicated build/test cost versus publish ESM only?
- Which `--traceResolution` observations are most useful for detecting package-graph regressions in large monorepos?
