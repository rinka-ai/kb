---
id: article-2026-06-11-building-a-good-vertical-agent
type: source
title: "Building a Good Vertical Agent"
path: raw/articles/user-provided/2026-06-11-building-a-good-vertical-agent.md
author: "BrainsAndTennis (@brainsandtennis)"
publisher: X
url: https://x.com/brainsandtennis/status/2065190286519906657
date_published: 2026-06-11
date_added: 2026-06-12
tags: [vertical-agents, context-engineering, agent-tools, agent-harnesses, agent-skills, spreadsheets]
status: active
quality: medium
summary: "Practitioner article arguing that a high-performing vertical agent is a faithful compression of its task distribution, using a spreadsheet agent to show L1/L2/L3 context tiers, code-mediated tool use, compressed read/write feedback, curated specs, and raw API escape hatches."
related: [context-engineering, agent-tools, agent-harnesses, agent-skills, resolvers]
---
...
- Publisher: X
- URL: https://x.com/brainsandtennis/status/2065190286519906657
- Local capture: /Users/josemanuelcerqueira/.codex/attachments/90e9f919-43ce-422d-a588-0bde0b9fabab/pasted-text.txt
- Verification note: X did not expose stable article text through direct web fetch during ingest. The tweet ID decodes to 2026-06-11T21:51:48Z; the source text below preserves the user-provided local capture.
- Verification boundary: claims about Shortcut's customer base, relative spreadsheet-agent accuracy, and internal experiments on tool count are practitioner claims from the source, not independently benchmarked in this ingest.

## TL;DR

This source is worth preserving because it turns generic context-engineering advice into a concrete vertical-agent design pattern: model performance depends on placing domain knowledge at the right context tier for the task distribution. The spreadsheet-agent example is especially useful because it shows how common read/write operations can be engineered as token-compressed, consequence-reporting wrappers, while occasional capabilities load through curated specs and rare long-tail capabilities remain reachable through a raw API reference plus a search skill.

## Key Claims

- A good vertical agent is "a faithful compression of its task distribution": its prompts, tools, docs, skills, and raw references should encode the frequency and importance of real user tasks.
- For a fixed model and environment, agent accuracy is strongly shaped by context quality: bloated context hides signal, missing context forces guessing, and both reduce product value.
- Context should be organized like a cache hierarchy:
  - L1: always-resident hot-path operations, kept tiny and highly engineered.
  - L2: important-but-occasional capabilities, loaded on demand through curated English specs or deferred tool schemas.
  - L3: complete raw references for the long tail, made searchable through a short skill or map.
- The common path deserves disproportionate engineering effort because every task pays for it. In the source's spreadsheet example, this means read/write operations that compress formulas, labels, styles, and diffs into model-usable feedback.
- A small model-facing tool surface can outperform a large menu of overlapping tools. The source reports using one `execute_code` tool whose code can call internal spreadsheet APIs, instead of exposing separate read, write, chart, pivot, and formatting tools.
- Read operations should be treated as compression problems: preserve relevant semantics while reducing repetitive formulas, nearby labels, and style information into compact representations.
- Write operations should report consequences, not just acknowledge success. Structured diffs should group, sample, and triage changed cells so errors such as invalid formulas are surfaced before the agent moves on.
- Curated L2 specs should encode canonical recipes and footguns, not only type signatures.
- L3 does not need to be pleasant as prompt context; it needs to be complete, reachable, and mineable in a bounded number of tool calls.
- Larger context windows do not eliminate hierarchy. As models improve, some L3 material can move to L2 and some L2 material can move to L1, but "right thing at the right time" remains the governing constraint.

## Important Details

- The author frames the basics of an agent as a loop around a model that calls tools until the task is done, then argues that durable advantage comes from domain-specific distribution knowledge and careful context/tool design rather than from the loop itself.
- The spreadsheet L1 example uses formula aliasing by normalizing repeated formulas to R1C1 form, nearby row/column context for labels and headers, and style compression by grouping identical formatting into compact range summaries.
- The spreadsheet write example returns a cell-diff summary that separates clean changes from cells needing review, including a `MUST FIX` category for suspicious or invalid results.
- The L2 example includes `getConditionalFormattingInfo()`, `getPivotTableInfo()`, `getChartInfo()`, `getDataValidationInfo()`, and `getAPIInfo("addSpanAt")` as on-demand prose/API helpers.
- The deferred-tool example uses `get_tool_info("web_search")` before `execute_tool("web_search", ...)`, treating fetched schemas as a session-scoped cache.
- The L3 example is a 70k-line raw Office.js or SpreadJS reference plus an `advanced-api` skill that teaches the agent grep recipes for methods, namespaces, types, and enums.
- The prompt-budget split mirrors the hierarchy: most prompt tokens for L1, a small allowlist/pointer layer for L2, and only a tiny signpost for L3.
- The article's strongest portable lesson is not spreadsheet-specific; it is the allocation rule: spend always-on context on high-frequency operations, use on-demand curated specs for occasional capabilities, and keep a complete raw escape hatch for rare tasks.

## Entities

- Author/account: BrainsAndTennis (@brainsandtennis)
- Product/company mentioned: Shortcut
- Domain example: spreadsheet agent
- APIs/surfaces mentioned: Office.js, SpreadJS, Excel plugin
- Functions/tools mentioned: `execute_code`, `getCellRange`, `setCell`, `getConditionalFormattingInfo`, `getPivotTableInfo`, `getChartInfo`, `getDataValidationInfo`, `getAPIInfo`, `get_tool_info`, `execute_tool`
- Concepts: vertical agents, context engineering, cache hierarchy, L1/L2/L3 context tiers, code-mediated tool use, formula aliasing, R1C1 normalization, structured diffs, deferred tools, curated specs, raw API references, skills

## My Notes

- This source should update existing context/tool/harness/skill pages rather than create a standalone concept page yet. "Vertical agent" can stay as a canonical phrase on [[resolvers]] or [[context-engineering]] until more sources justify a dedicated page.
- Best durable addition to the KB: the task distribution is the unit being compressed, not merely the prompt or session history.
- This strengthens the KB's existing progressive-disclosure thesis with a concrete allocation heuristic: L1 for hot path, L2 for one-step curated misses, L3 for bounded raw-reference search.
- The spreadsheet examples add unusually concrete evidence for "tool results are context design": read wrappers and write diffs are model-facing semantic compression, not neutral API plumbing.
- The one-tool claim aligns with [[agent-tools]] code-mediated orchestration patterns, but should stay framed as a design tradeoff. Other domains may need multiple model-facing tools for safety, approvals, or affordance clarity.
- Treat Shortcut accuracy/customer claims as anecdotal practitioner evidence unless later backed by public benchmarks or customer case studies.

## Open Questions

- What measurement loop did Shortcut use to decide which spreadsheet capabilities belong in L1 versus L2?
- How general is the "one execute tool" pattern outside spreadsheet/code-friendly domains, especially when permissions, approvals, or side effects need separate affordances?
- Which parts of spreadsheet context compression are lossless, and which are lossy but pragmatically useful?
- How should agent builders detect when an L2 spec has become common enough to promote into L1?
- Can the L1/L2/L3 hierarchy be evaluated through task-distribution-weighted accuracy and cost rather than anecdotal latency or token savings?
