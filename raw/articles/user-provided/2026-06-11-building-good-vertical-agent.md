---
id: article-2026-06-11-building-good-vertical-agent
type: source
title: "Building a Good Vertical Agent"
path: raw/articles/user-provided/2026-06-11-building-good-vertical-agent.md
author: "Peter Wang (@BrainsAndTennis)"
publisher: "X / user-provided paste"
url: https://x.com/brainsandtennis/status/2065190286519906657
date_published: 2026-06-11
date_added: 2026-06-12
tags: [agents, vertical-agents, context-engineering, agent-tools, agent-skills, code-execution, spreadsheets]
status: active
quality: medium
summary: Peter Wang argues that high-performing vertical agents are faithful compressions of their domain task distribution, using L1/L2/L3 context tiers, code-mediated tool access, compressed read/write wrappers, deferred specs, and raw-reference escape hatches.
related: [context-engineering, agent-tools, agent-skills, agent-harnesses, llm-agents, workflows]
---
...
- Path: raw/articles/user-provided/2026-06-11-building-good-vertical-agent.md
- Local source file: `/Users/josemanuelcerqueira/.codex/attachments/5fbc6b52-c253-43e2-8a84-5c5269cd432a/pasted-text.txt`
- Author: Peter Wang (@BrainsAndTennis)
- Published: 2026-06-11
- Publisher: X / user-provided paste
- URL: https://x.com/brainsandtennis/status/2065190286519906657
- Article title: Building a Good Vertical Agent
- Source handling note: The article body below preserves the user-provided pasted text. X metadata identified the author and status date, but the clean source body used for this ingest is the local attachment.
- Verification boundary: Claims about Shortcut's spreadsheet-agent accuracy and deployment inside hedge funds are self-reported by the author and should be treated as practitioner evidence, not independently verified benchmark or customer proof.

## TL;DR

This article frames a good vertical agent as a faithful compression of its domain task distribution. The strongest reusable pattern is to allocate context like a memory hierarchy: L1 for high-frequency operations that must be always resident and highly compressed, L2 for important-but-occasional curated specs fetched in one step, and L3 for complete raw references plus a small skill that teaches the agent how to search them.

The spreadsheet examples make the pattern concrete: use one code-execution substrate instead of many overlapping tools; make common read/write operations return compressed, semantically enriched, consequence-reporting outputs; defer richer docs and tool schemas until needed; and keep a raw API escape hatch so rare tasks are still reachable. The broader KB lesson is that model quality in vertical agents is often driven by domain-specific context and tool-result engineering, not only by model choice.

## Key Claims

- Building a basic agent loop is no longer the hard part; domain performance depends on careful context, tool, artifact, and wrapper design.
- A good agent is a faithful compression of its task distribution.
- With the model fixed, accuracy depends heavily on context quality: bloated context buries signal, while missing context forces guessing.
- Context should be tiered like a CPU cache: L1 always resident for the common case, L2 fetched on demand for important occasional capabilities, and L3 raw references for the long tail.
- The design goal is not "make everything available" but minimize context spent per task across the actual task distribution.
- A single `execute_code` tool can outperform many overlapping model-facing tools by moving API composition into code and reducing tool-selection confusion.
- High-frequency operations deserve disproportionate engineering because the agent pays their cost on every task.
- Spreadsheet range reads should compress formulas, add surrounding row/column context, and compress styles so the model sees the semantic table rather than raw cells alone.
- Spreadsheet writes should return compressed diffs plus suspicious-change triage, so the feedback loop says what changed and what likely went wrong.
- L2 capability specs should be curated prose that teaches canonical recipes and gotchas, not just type signatures.
- Deferred tool schemas can be treated as a session-scoped cache: fetch the schema once, then allow execution.
- L3 raw references are acceptable if paired with a small search skill that makes the complete API findable in a bounded number of calls.
- As models improve, capabilities may migrate between tiers, but the hierarchy remains because context and attention stay scarce relative to the domain's full knowledge surface.

## Important Details

- The article uses Shortcut's spreadsheet agent as the running example and says its accuracy is the main reason customers choose it despite weaker distribution than Microsoft or Anthropic.
- The proposed context tiers map to prompt budget: most prompt space goes to the common 80%, a small always-on pointer layer signposts the next 15%, and the long-tail raw reference stays almost entirely off-context.
- L1 examples include core read/write spreadsheet functions, execution contracts, key types, safety guidance, and compact consequence-reporting wrappers.
- Formula aliasing normalizes repeated formulas to R1C1-style patterns and emits aliases such as `F1` instead of repeating hundreds of near-identical formulas.
- Row/column context is added by scanning nearby labels and headers, reducing the chance that the model misinterprets naked cell values.
- Style compression groups identical formatting across connected ranges and emits compact descriptions instead of per-cell formatting.
- Write diffs are grouped, sampled, counted, and triaged; suspicious values such as invalid formulas, hardcoded numbers, or implausible percentages are elevated.
- L2 examples include curated specs for conditional formatting, pivot tables, charts, data validation, and single-function docs via `getAPIInfo(...)`.
- The pivot-table example emphasizes gotchas that raw APIs often fail to teach, such as suspending/resuming layout and using raw integer aggregations when friendly enums are missing at runtime.
- L2 deferred tools use a two-step contract: fetch tool info first, then execute only if the tool was fetched.
- L3 consists of complete Office.js or SpreadJS references on disk plus an `advanced-api` skill that maps grep/search recipes over the raw reference structure.
- The escape hatch rule is explicit: if wrapped APIs do not cover a need, use the raw API rather than compromising on the requested behavior.

## Entities

- Author: Peter Wang (@BrainsAndTennis)
- Product/company mentioned: Shortcut
- Platforms/tools mentioned: Codex, Claude Code, Pi, Microsoft, Anthropic, Office.js, SpreadJS, Excel plugin
- Agent design elements: system prompt, tools, artifacts, skills, curated docs, raw references, code execution
- Context tiers: L1, L2, L3
- Example APIs/functions: `execute_code`, `getCellRange`, `setCell`, `getConditionalFormattingInfo`, `getPivotTableInfo`, `getChartInfo`, `getDataValidationInfo`, `getAPIInfo`, `get_tool_info`, `execute_tool`

## My Notes

- The best KB update is the distributional framing: vertical-agent context should encode the frequency and value distribution of domain tasks, not merely maximize information availability.
- This sharpens [[context-engineering]] by giving a concrete allocation rule: resident context for the common path, one-step specs for important occasional paths, and bounded-search raw references for the long tail.
- It sharpens [[agent-tools]] by arguing for code-mediated tool composition when many overlapping tools degrade model accuracy.
- It sharpens [[agent-skills]] by positioning a skill as the map over a raw reference, not only a high-level workflow procedure.
- It sharpens [[agent-harnesses]] because the harness owns the read/write wrappers, structured diffs, deferred tool-schema cache, and raw-reference path that make the model accurate in a domain.
- Treat the numeric deployment and performance claims as claims from a builder, not as third-party-validated evidence. The architecture patterns are useful even if the commercial claims remain unverified.

## Open Questions

- How much of Shortcut's accuracy gain comes from the single code-execution substrate versus the spreadsheet-specific read/write compression wrappers?
- What empirical curve did Shortcut observe between tool count and model accuracy, and does it transfer outside spreadsheet domains?
- How should teams measure whether a capability belongs in L1, L2, or L3 beyond intuition about frequency and value?
- What safeguards are needed when L3 raw-reference mining lets an agent reach obscure APIs with poorly understood side effects?
- How should context-tier assignments be updated as model strength, user task distribution, and API surfaces change?
