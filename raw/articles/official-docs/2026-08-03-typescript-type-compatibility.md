---
id: 2026-08-03-typescript-type-compatibility
type: source
title: TypeScript Type Compatibility
path: raw/articles/official-docs/2026-08-03-typescript-type-compatibility.md
author: TypeScript Team
publisher: Microsoft
url: https://www.typescriptlang.org/docs/handbook/type-compatibility.html
date_published:
date_added: 2026-08-03
tags: [typescript, type-system, structural-typing, assignability, variance, generics, soundness, api-design]
status: active
quality: high
summary: The official TypeScript handbook defines structural assignability across objects, functions, classes, and generics, and documents deliberate soundness trade-offs made for common JavaScript patterns.
related: [typescript-structural-compatibility, typescript-control-flow-narrowing, typescript-module-systems, internal-engineering-conventions]
---
...
- Publisher: Microsoft
- URL: https://www.typescriptlang.org/docs/handbook/type-compatibility.html
- Provenance: Captured through `bun run kb:ingest --url` on 2026-08-03, then relocated into the established `official-docs` collection and curated without changing `## Source Text`. The canonical URL returned HTTP 200 during curation.

## TL;DR

TypeScript primarily decides compatibility by comparing the structure a target requires with the structure a source provides, matching JavaScript's object- and callback-heavy style rather than requiring nominal declarations. This convenience is intentionally not fully sound: callback parameters, optional/rest parameters, `any`, and unchecked declarations can admit runtime risk, so public APIs need strict compiler settings, type-level regression tests, and runtime validation where values cross untrusted boundaries.

## Key Claims

- TypeScript uses structural subtyping: a source object is compatible when it has recursively compatible members for every member required by the target.
- Extra members usually do not prevent assignment, although directly typed object literals receive an additional known-property check.
- A function with fewer required parameters can replace one with more because JavaScript callbacks commonly ignore arguments; return types flow in the opposite direction, requiring the source result to satisfy the target result.
- Function-parameter bivariance and optional/rest compatibility permit useful JavaScript patterns that are not provably safe; `strictFunctionTypes` can reject relevant parameter errors.
- Every target overload must be matched by a compatible source signature.
- Class instances are structurally compared except that private or protected members must originate from the same class hierarchy, creating a limited nominal boundary.
- A generic parameter affects compatibility only when it changes the resulting member structure; an unused type parameter does not distinguish otherwise empty shapes.
- Practical compatibility is governed mainly by assignment compatibility, including special rules around `any`, `unknown`, `never`, nullish values, and enums.

## Important Details

- **Type/API boundary:** exported interfaces are shape contracts, not declarations of identity. Two independently declared values can be interchangeable if their public members match, while an accidental member change can alter assignability without any explicit inheritance relationship.
- **Runtime-versus-type boundary:** compatibility is erased. It cannot establish that JSON, environment variables, database rows, plugin values, or callback payloads actually have the accepted shape at runtime; those boundaries still require parsing and validation.
- **Package graph:** published declarations define the shape consumers compare. Stale or inaccurate declarations can make an incompatible runtime look safe, while duplicated class declarations with private/protected members can become incompatible because member origin matters. Test the packaged public surface described by [[typescript-module-systems]].
- **Composition/lifecycle ownership:** API owners should define callback input breadth and output guarantees; callers should not narrow a handler's accepted input beyond what the producer may emit. Domain owners should decide where structural interchangeability is desirable and where an explicit discriminant or opaque/nominal technique is needed.
- **Tests:** add compile-time fixtures for allowed and rejected assignments, callback variance, overload coverage, generic phantom parameters, and public declaration imports. Pair them with runtime tests wherever external values or callbacks can violate the static promise.
- **Performance:** the chapter says compatibility checks recurse through members but provides no compiler, editor, or runtime benchmark. Measure large generated types and declaration graphs with compiler diagnostics rather than inferring a cost from the model.
- **Transfer limits:** the handbook explains broad compatibility rules, not every configuration-, syntax-, or version-specific exception. Strictness flags change which risky assignments compile, and TypeScript deliberately prioritizes common JavaScript behavior over complete soundness.
- **Practical implication:** expose the smallest truthful structural contract, enable strict checking, keep callbacks broad enough for every value the producer may send, validate unknown runtime values, and lock intended assignability into package-level type tests.

## Entities

- Organization: Microsoft TypeScript Team
- Languages/runtimes: TypeScript, JavaScript
- Type-system mechanisms: structural subtyping, assignment compatibility, function variance, overloads, generics, private/protected member origin, `any`, `unknown`, `never`
- Compiler option: `strictFunctionTypes`

## My Notes

- Structural typing is an API-composition feature and a change-amplification risk: consumers can depend on shape without coupling to a declaration, but apparently local member edits may alter compatibility across distant call sites.
- Variance is easiest to review from ownership: the producer controls which arguments may arrive; the consumer controls what result it promises. Callback types should preserve those directions instead of relying on casts.
- Empty or phantom generic parameters do not create distinct domain types. If identity matters, the type must contain a differentiating member or use another deliberate nominal pattern.
- A clean typecheck proves compatibility with the declarations the compiler saw, not correspondence between those declarations and deployed JavaScript.

## Open Questions

- Which current TypeScript syntax and strictness settings best express variance expectations for reusable library APIs without relying on method-specific compatibility exceptions?
- Which type-test harness provides the clearest package-consumer regression tests for the KB's preferred Bun/TypeScript stack?
- When do generated structural types and overload sets materially affect compiler or language-service latency?
