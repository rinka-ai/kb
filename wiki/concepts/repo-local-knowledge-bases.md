...
source_count: 4
summary: Repo-local knowledge bases are committed project-memory layers that help coding agents and humans reuse product, codebase, design, and decision knowledge without rereading the whole source tree.
...
A repo-local knowledge base is a committed project-memory layer inside or next to a codebase. It preserves product truth, codebase conventions, design rules, raw snapshots, decision records, glossary entries, loadouts, and activity logs so future agents and humans can orient without rereading every file. It differs from private agent memory because it is inspectable, versioned, and shared with the repo.
