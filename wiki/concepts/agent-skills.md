...
source_count: 27
summary: Agent skills are reusable procedural capability modules that package task-specific guidance, examples, scripts, setup state, hooks, verification habits, and bounded search maps for long-tail references while keeping invocation, evidence, and mutation boundaries explicit.
...
- use compact map skills for long-tail references: teach the agent file structure, search commands, type-resolution paths, and enum lookup recipes instead of loading the raw reference into prompt context
