---
id: 2026-08-02-typescript-narrowing
type: source
title: TypeScript Narrowing
path: raw/articles/official-docs/2026-08-02-typescript-narrowing.md
author: TypeScript Team
publisher: Microsoft
url: https://www.typescriptlang.org/docs/handbook/2/narrowing.html
date_published:
date_added: 2026-08-02
tags: [typescript, type-system, narrowing, control-flow-analysis, discriminated-unions, type-guards, never, api-design]
status: active
quality: high
summary: The official TypeScript handbook explains how runtime JavaScript checks drive static narrowing, how discriminated unions make state-dependent fields representable, and how never-based exhaustiveness checks turn missing cases into compile-time failures.
related: [typescript-control-flow-narrowing, codebase-architecture, internal-engineering-conventions]
---
...
- Publisher: Microsoft
- URL: https://www.typescriptlang.org/docs/handbook/2/narrowing.html
- Provenance: Captured through `bun run kb:ingest --url` on 2026-08-02, then relocated into the established `official-docs` collection and curated without changing `## Source Text`.

## TL;DR

TypeScript overlays flow-sensitive static analysis on ordinary JavaScript runtime checks. Built-in guards, assignments, reachability, predicates, discriminated unions, and `never` let APIs represent valid states precisely and force callers to handle new variants, but none of these checks validate unknown runtime data by themselves.

## Key Claims

- TypeScript narrows a declared union to a more specific observed type by following runtime checks and possible execution paths.
- `typeof`, truthiness, equality, `in`, and `instanceof` are recognized guards, including JavaScript quirks such as `typeof null === "object"` and falsy empty strings.
- Assignment changes a variable's observed type at that program point, while later assignability is still checked against its declared type.
- Control-flow analysis uses reachability, returns, branches, assignments, and re-merging paths to compute a variable's type at each location.
- User-defined predicates can expose reusable narrowing logic, but the predicate's implementation is a developer-authored promise that must stay aligned with runtime behavior.
- A discriminated union models each valid state as a separate object type with a shared literal-valued field; checking that field narrows the whole object without non-null assertions.
- Once all union members are eliminated, TypeScript uses `never`; assigning a supposedly remaining case to `never` makes a switch fail to compile when a new variant is not handled.

## Important Details

- **Type/API boundary:** prefer unions of complete variants over one interface with a tag plus unrelated optional fields. The former lets the public type encode which payload belongs to each state and makes downstream switches mechanically checkable.
- **Runtime-versus-type boundary:** narrowing reasons from JavaScript operations that actually execute, but a TypeScript annotation, predicate, assertion function, or discriminated union does not parse or validate untrusted input. Validate at HTTP, queue, storage, environment, and provider boundaries before treating data as the union.
- **Package graph:** narrowing changes no runtime import or package edge and does not solve module-resolution or declaration-publication problems. Exported predicates and unions are API contracts whose declarations and implementation must still travel through the package graph described by [[typescript-module-systems]].
- **Composition/lifecycle ownership:** the domain layer should own discriminants and legal variants; boundary adapters should validate and normalize external values; orchestration code should exhaustively transition or handle domain states. Do not let provider-specific status strings become the canonical discriminant unless they are genuinely the domain contract.
- **Tests:** compile-time fixtures should verify accepted and rejected variants and exhaustive consumers; runtime tests should verify predicate/validator behavior, every state transition, malformed input, JavaScript edge cases, and the default/unreachable path where applicable.
- **Performance:** the handbook makes no compiler or runtime benchmark claim. Guards execute as ordinary JavaScript; any compiler-performance claim about very large unions or control-flow graphs requires measurement rather than inference from this chapter.
- **Transfer limits:** narrowing is flow-sensitive, not proof of external truth. Truthiness may drop valid falsy values, optional properties can appear on both sides of an `in` check, `instanceof` depends on prototype identity, and a wrong custom predicate can produce unsound downstream assumptions.
- **Practical implication:** encode state so invalid field combinations are absent from the union, narrow with real runtime evidence, validate unknown data before admission, and place an exhaustive `never` check wherever every variant must be handled.

## Entities

- Organization: Microsoft TypeScript Team
- Language/runtime: TypeScript, JavaScript
- Type-system mechanisms: unions, narrowing, control-flow analysis, type predicates, discriminated unions, `never`
- Runtime guards: `typeof`, truthiness, equality, `in`, `instanceof`

## My Notes

- The most reusable API-design lesson is not merely “use type guards”; it is to make legal lifecycle states separate union members so control flow can preserve correlations between a discriminant and its payload.
- Non-null assertions often signal that the data model has hidden a correlation the type system could represent directly.
- Exhaustiveness checks make domain evolution visible to every consumer, but only if teams do not silence the new compile error with broad casts or a catch-all branch.
- Boundary validation and internal narrowing are complementary: validation establishes that runtime data belongs to the domain union; narrowing safely consumes that admitted value.

## Open Questions

- Which compile-time test style (`@ts-expect-error`, declaration tests, or dedicated type-test tooling) best protects exported discriminated unions in this KB's preferred Bun/TypeScript stack?
- When do very large unions or deeply nested guards materially affect TypeScript language-service latency?
- Which domain states deserve closed exhaustive unions, and which integration-facing statuses must remain open to unknown future provider values?
...
tsfunction padLeft(padding: number | string, input: string): string {  throw new Error("Not implemented yet!");}Try
...
ts// both of these result in 'true'Boolean("hello"); // type: boolean, value: true!!"world"; // type: true,    value: trueThis kind of expression is always truthy.2872This kind of expression is always truthy.Try
...
tsfunction multiplyAll(  values: number[] | undefined,  factor: number): number[] | undefined {  if (!values) {    return values;  } else {    return values.map((x) => x * factor);  }}Try
Equality narrowing
...
tstype Fish = { swim: () => void };type Bird = { fly: () => void };function move(animal: Fish | Bird) {  if ("swim" in animal) {    return animal.swim();  }  return animal.fly();}Try
...
tstype Fish = { swim: () => void };type Bird = { fly: () => void };type Human = { swim?: () => void; fly?: () => void };function move(animal: Fish | Bird | Human) {  if ("swim" in animal) {    animal;      (parameter) animal: Fish | Human  } else {    animal;      (parameter) animal: Bird | Human  }}Try
...
tslet x = Math.random() < 0.5 ? 10 : "hello world!";   let x: string | numberx = 1;console.log(x);           let x: numberx = "goodbye!";console.log(x);           let x: stringTry
...
tslet x = Math.random() < 0.5 ? 10 : "hello world!";   let x: string | numberx = 1;console.log(x);           let x: numberx = true;Type 'boolean' is not assignable to type 'string | number'.2322Type 'boolean' is not assignable to type 'string | number'.console.log(x);           let x: string | numberTry
...
For example
tsfunction padLeft(padding: number | string, input: string) {  if (typeof padding === "number") {    return " ".repeat(padding) + input;  }  return padding + input;}Try
...
Both interpretations are correct, but only the union encoding of Shape will cause an error regardless of how strictNullChecks is configured.
