...
- Memory reads should also verify integrity at retrieval time; a hash checked only at storage time does not catch later tampering.
- Source attribution should travel with memory elements so untrusted external context, tool output, user preference, and verified internal fact do not collapse into one authority level.
- Memory retention is a security control: high-risk, unverified, or externally sourced context should expire faster than core operational facts.
- Poisoned-memory recovery needs prepared rollback, quarantine, and purge criteria rather than ad hoc deletion during an incident.
